...
} from "../../extensions/accounts/storage.js";
...
test("provider-scoped storage preserves independent active accounts and OAuth metadata", async () => {
	...
	assert.equal(
		Object.hasOwn(
			stored.providers.anthropic?.accounts.work ?? {},
			"optionalMetadata",
		),
		false,
	);
	...
	assert.deepEqual(
		stored.providers["github-copilot"]?.accounts.personal?.availableModelIds,
...
test("parsed provider and account maps treat prototype-like names as own properties", () => {
	...
	const constructorCredential = Object.getOwnPropertyDescriptor(
		accounts,
		"constructor",
	)?.value as { access?: string } | undefined;
		...
		() =>
			parseAccountsData(
				JSON.stringify({
					version: 1,
					providers: {
						anthropic: { accounts: { work: { access: "a", expires: 1 } } },
					},
				}),
			),
...
test("account storage keeps the previous version's default lock path", async () => {
	const dir = await mkdtemp(
		join(tmpdir(), "pi-accounts-store-compatible-lock-"),
	);
...
test("sync missing account reads wait on an existing default lock", async () => {
	const dir = await mkdtemp(
		join(tmpdir(), "pi-accounts-store-compatible-sync-lock-"),
	);
	const file = join(dir, ACCOUNTS_FILE);
	try {
		await mkdir(`${file}.lock`);
		const backend = new FileAccountStorageBackend(file, {
			syncLockTimeoutMs: 20,
		});
		let entered = false;
...
test("async account reads reject contained lock compromise errors", async () => {
	const dir = await mkdtemp(
		join(tmpdir(), "pi-accounts-store-compromised-read-"),
	);
	const file = join(dir, ACCOUNTS_FILE);
	try {
...
test("migration copies released Codex schema into provider state and retains rollback source", async () => {
	...
	try {
		...
		assert.equal(
			migrated.providers["openai-codex"]?.accounts.work?.accountId,
			"acct",
		);
	} finally {
		await rm(dir, { recursive: true, force: true });
...
test("concurrent migrations serialize and install one complete canonical file", async () => {
	const dir = await mkdtemp(
		join(tmpdir(), "pi-accounts-concurrent-migration-"),
	);
	const legacy = join(dir, "pi-codex-accounts.json");
	const canonical = join(dir, ACCOUNTS_FILE);
	try {
		...
		assert.equal(
			parseAccountsData(await readFile(canonical, "utf8")).providers[
				"openai-codex"
			]?.active,
			"work",
		);
...
test("migration recovers from stale interrupted temporary files", async () => {
	const dir = await mkdtemp(
		join(tmpdir(), "pi-accounts-interrupted-migration-"),
	);
	const legacy = join(dir, "pi-codex-accounts.json");
	const canonical = join(dir, ACCOUNTS_FILE);
	...
	try {
		...
		assert.equal(
			(await migrateLegacyCodexAccountsFile(legacy, canonical)).status,
			"migrated",
		);
