...
async function getWsUrl() {
	...
	throw new Error(
		`CDP HTTP discovery failed and no DevToolsActivePort found. Enable remote debugging or set CDP_PORT/CDP_PORT_FILE. Tried: ${errors.join("; ")}`,
	);
}
...
async function waitForOpenedTarget(
	cdp,
	targetId,
	requestedUrl,
	timeout = 5000,
	...
	if (requestedUrl === "about:blank")
		return { targetId, title: requestedUrl, url: requestedUrl };
...
async function shotElementStr(cdp, sid, selector, filePath, targetId) {
  ...
  `;
	...
	const out =
		...
		resolve(
			RUNTIME_DIR,
			`screenshot-${(targetId || "unknown").slice(0, 8)}-element.png`,
		);
	writeFileSync(out, Buffer.from(data, "base64"));
	return screenshotReport(out, dpr, [
...
async function typeStr(cdp, sid, text) {
  ...
  `,
	...
	if (after.value === focusState.before) {
		throw new Error(
			`Input.insertText completed but focused <${focusState.tag}> did not change`,
		);
	}
...
async function main() {
	...
	if (cmd === "open") {
		...
		console.log(
			'Note: Chrome may request "Allow debugging?" approval on first access.',
		);
		return;
	}
