...
async function backendCenter(cdp, sid, backendNodeId, scroll = true) {
	...
	if (!point?.ok)
		throw new Error(point?.error || "Element has no clickable box");
	return point;
}

async function selectorBackendNode(cdp, sid, selector) {
	await cdp.send("Runtime.enable", {}, sid);
	const selected = await cdp.send(
		"Runtime.evaluate",
		{
			expression: `(() => {
	...
	if (selected.exceptionDetails) {
		throw new Error(
			selected.exceptionDetails.exception?.description ||
				selected.exceptionDetails.text,
		);
	}
...
async function htmlRefStr(cdp, sid, elementRefs, id) {
	const ref = requireElementRef(elementRefs, id);
	const result = await withBackendObject(
		cdp,
		sid,
		ref.backendNodeId,
		(objectId) =>
...
async function pressAndRelease(cdp, sid, point) {
	...
	try {
		...
		if (pressed)
			await dispatchMouse(cdp, sid, "mouseReleased", point).catch(() => {});
	}
}
...
async function typeRefStr(cdp, sid, elementRefs, id, text) {
	...
	return withBackendObject(cdp, sid, ref.backendNodeId, async (objectId) => {
		...
		if (!focusState?.ok)
			throw new Error(focusState?.error || "Element is not editable");
		const activeBefore = (
			await cdp.send(
				"Runtime.callFunctionOn",
				{
		...
		if (!after.changed)
			throw new Error(
				`Input.insertText completed but referenced <${focusState.tag}> did not change`,
			);
