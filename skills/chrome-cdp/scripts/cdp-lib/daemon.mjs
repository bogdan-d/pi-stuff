	...
	async function handleCommand({ cmd, args, targetRef }) {
		...
		try {
			...
			switch (cmd) {
				...
				case "snap":
				case "snapshot":
					result = await snapshotStr(
						cdp,
						sessionId,
						elementRefs,
						targetRef || (await getTargetRef(cdp, targetId)),
						args[0],
						args[1],
					);
					break;
				case "find":
					result = await findStr(
						cdp,
						sessionId,
						elementRefs,
						targetRef || (await getTargetRef(cdp, targetId)),
				...
				case "html":
					...
					break;
				case "htmlref":
					result = await htmlRefStr(cdp, sessionId, elementRefs, args[0]);
					break;
				case "nav":
				case "navigate":
				...
				case "click":
					...
					break;
				case "clickref":
					result = await clickRefStr(cdp, sessionId, elementRefs, args[0]);
					break;
				case "clickxy":
...
async function getOrStartTabDaemon(targetId) {
	...
	throw new Error(
		`Tab bridge did not become ready within ${DAEMON_CONNECT_TIMEOUT / 1000}s. The target may have closed or Chrome may not be answering.`,
	);
}
