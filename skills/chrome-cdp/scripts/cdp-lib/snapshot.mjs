...
async function snapshotData(cdp, sid, elementRefs, options = {}) {
	...
	function visit(node, depth, parentName = "") {
		...
		if (!node.ignored && shouldShowAxNode(node, true)) {
			...
			if (interactive) {
				...
				addLine(
					`[${element.id}] ${role}${name ? ` ${name}` : ""}${value === "" || value == null ? "" : ` = ${JSON.stringify(value)}`}`,
					element,
					"interactive",
				);
				renderedName = name;
	...
	const metadata = JSON.parse(
		await evalStr(cdp, sid, "({title: document.title, url: location.href})"),
	);
	...
	const visibleIds = new Set(
		content.map((line) => line.element_id).filter(Boolean),
	);
	const visibleElements = elements.filter((element) =>
		visibleIds.has(element.id),
	);
...
async function snapshotStr(
	...
	return JSON.stringify(
		await snapshotData(cdp, sid, elementRefs, {
			refId,
